...
if '[' "$tmux" '=' '1' ']'; then
  'command' 'awk' "/Mark up shell's output/ {print \"# Start tmux if not already in tmux.\"; print \"zstyle ':z4h:' start-tmux       command tmux -u new -A -D -t z4h\"; print \"\"} 1" "$zshrc" >"$zshrc.bak"
else
