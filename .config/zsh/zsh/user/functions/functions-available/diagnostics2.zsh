...
# vim:ft=zsh sw=2 sts=2 et foldmarker=function,function:foldmethod=marker
...
#       Verbosity levels:
#        0 - Basic info, shell state, zsh configuration, git state
...
function zsh_diagnostic_dump() {
  emulate -L zsh

  builtin echo "Generating diagnostic dump; please be patient..."
  
  local thisfcn=zsh_diagnostic_dump
  local -A opts
  local opt_verbose opt_noverbose opt_outfile
  local timestamp=$(date +%Y%m%d-%H%M%S)
  local outfile=zsh_diagdump_$timestamp.txt
  ...
  # captured cleanly
  _zsh_diag_dump_one_big_text &> "$outfile"
  if [[ $? != 0 ]]; then
  ...
  builtin echo
  builtin echo To share this with zsh developers, post it as a gist on GitHub 
  builtin echo at "https://gist.github.com" and share the link to the gist.
  builtin echo
  builtin echo "WARNING: This dump file contains all your zsh and zsh configuration files,"
  builtin echo "so don't share it publicly if there's sensitive information in them."
  builtin echo

}

function _zsh_diag_dump_one_big_text() {
  ...
  builtin echo
  _zsh_diag_dump_os_specific_version
  builtin echo
  ...
  _zsh_diag_dump_check_core_commands || return 1
  builtin echo  
  ...
  builtin set | command grep -a '^\(ZSH\|plugins\|TERM\|LC_\|LANG\|precmd\|chpwd\|preexec\|FPATH\|TTY\|DISPLAY\|PATH\)\|zsh'
  builtin echo
  ...
  if [[ $verbose -ge 1 ]]; then
    for cfgfile in $cfgfiles; do
      _zsh_diag_dump_echo_file_w_header $cfgfile
    done
  fi
  builtin echo
  ...
  if [[ $verbose -ge 2 ]]; then
    for dumpfile in $dumpfiles; do
      _zsh_diag_dump_echo_file_w_header $dumpfile
    done
  fi

}

function _zsh_diag_dump_check_core_commands() {
  ...
  # "history" is also excluded because zsh is known to redefine that
...
function _zsh_diag_dump_echo_file_w_header() {
  local file=$1
...
function _zsh_diag_dump_os_specific_version() {
