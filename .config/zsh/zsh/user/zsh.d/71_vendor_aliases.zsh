...
if [[ "${commands[lsd]}" ]]; then
  ...
  if islsd22; then
    ...
    alias lls="printf '\e[0;1;38;5;93m🍆💦🍑\e[0;1;3;4;38;5;199mThis could take a quick second\e[0m\e[0;1;38;5;93m🍄🦄🐉\e[0m\t'; date +'%Y%m%d_%H%M%S';  lsd --long --sort=size --header --color=always --total-size --date=+'%Y%m%d%H%M%S' --almost-all --dereference --blocks 'size,date,name' --reverse --permission octal 2>/dev/null"
...
#   alias ls-ll="exa --sort=type --tree --recurse --long --all"
  alias lst="exa --sort time --colour-scale --long --time-style=long-iso --no-user --no-permissions --header --git --no-icons  --accessed --modified"
  alias exa="exa --color always --color-scale --icons"
  alias l1='exa  --oneline --all --header'                        # List All On One Line
  alias lll='exa --long --octal-permissions --all --header --no-time --no-user --git --sort=type --colour-scale'
