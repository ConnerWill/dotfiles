...
function c(){
  ...
  [[ -z "$PAGER" ]] \
    && local PAGER="less"
  find . -readable -type f -exec "$PAGER" {} \;
}
...
#  alias l='clear ; echo -e "▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂" ; pwd ; echo -e "██████████████████████████████████" ; lsd --color always --oneline --all ; echo -e "▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂\n"   ' # List All On One Line
  ...
  alias lls="tput smcup ; clear ; printf 'o boi ...   This could take a  quick second ﭂ ' ; lsd --long --sort=size --date=+'%Y-%m-%d.%H%M%S' --hyperlink=always --permission=octal --no-symlink --color=always --total-size --almost-all --reverse"
...
### [ cp ]
alias cp="cp --verbose"
### Confirm Before Overwrite/Deletion ###
alias cp='cp --interactive --verbose'
# cp but with progress
alias cpv='rsync --archive --human-readable --info=progress2'
# cp but with progress and verbose
alias cpvvv='rsync --archive --human-readable --info=ALL'
...
alias update-repo='sudo pacman --sync --refresh --verbose'
# Update the local database of PGP keys used by the package maintainers.
alias update-repo-pgp='sudo pacman --sync --needed archlinux-keyring --verbose'
...
# Change Permissions To Give User Execute Permissions
...
#alias gccb="git git clone --no-checkout --verbose $(xclip -selection clipboard -out)"
alias git-clone-nocheckout="git clone --no-checkout --verbose"
## Better bulk clone: 'githubcloner'
...
alias chkpw="firefox --private-window '$HOME/security/pws/password-strength/index.html'"
...
alias docker-interactive="echo -e '\n\tdocker search \$1 \n\tdocker pull \$1 \n\tdocker image list --all \$1 \n\tdocker container create --init \$1\n\tdocker container run --interactive \$1\n'"

### List Non-Free Software
#alias list-nonfree-software='clear && figlet -f "ANSI Shadow" "VRMS" | lolcat && echo "Virtual Richard M. Stallman for Arch Linux" | lolcat && tput bold && tput setaf 196 && echo "\n\n---------------------------- Non Free Packages Installed ----------------------------\n" && tput sgr0 &&  vrms && tput bold && tput setaf 196 && echo "\n------------------------------------------------------------------------------------" && tput bold && tput setaf 226 && echo "\n\n--------------------- Installed Packages With Unknown Licenses ---------------------\n" && tput sgr0 &&  vrms -u && tput bold && tput setaf 226 && echo "\n------------------------------------------------------------------------------------"'
...
alias ssharchserver="ssh root@connerwill.com"
alias ssh-copypublickey='copysshkeypublic'
alias ssh-showpublickey='cat ~/.ssh/id_rsa.pub'
alias ssharchpi="ssh alarm@192.168.64.176"
alias sshoctopi="tmux new-session ssh pi@192.168.64.148"
alias ssh-octopi="ssh -l pi -p 22 $OCTOPIIP"
...
### [~]............ MPV
### [=]==================================[=]
alias mpv-fun='clear && echo -n "\n\n\n\t\t\t${fg_green}ENTER URL:\t\t\t${reset_normal}" && read UserInputURL && clear && echo "\n\n\n\t\t\t${fg_green}OPENING URL:\t\t\t${fg_yellow}$UserInputURL${reset_normal}\n\n\n\t\t\t" && mpv $UserInputURL'
...
# alias external-ip="upnpc -s | sed -n -e 's/^ExternalIPAddress = \([0-9.]*\)$/\1/p'"
...
alias black-squidward-ascii="image2ascii -recwidth -recheight -bright 6 -contrast 104 -gamma 0.8 -nodither -boldmul 45 -dimmul 33 /home/dampsock/Downloads/squidward-black.png"

alias pixel-art-editor='pxltrm'

alias show-archascii-all='tulizu list arch-logo-lettering-condensed s c1 && tulizu list arch-big-logo s c1 && tulizu list arch-logo-medium-bang s c1'
alias show-archascii-medium='tulizu list arch-logo-lettering-condensed s c1'
alias show-archascii-large='tulizu list arch-big-logo s c1'
alias show-archascii-small='tulizu list arch-logo-medium-bang s c1'

alias ASCII-ART-SHOW='bb'
alias ASCII-FIRE="aafire -extended -random 20 -boldmul 2.7 -gamma .4"
...
alias scp-connerwill-send="scp -P 420 -v $1 root@connerwill.com:Downloads/"


#alias ssh-keygen-ed25519="echo -n 'Enter Comment (no_spaces): ' && read sshcomment && KeyName='$HOST-$(date +%Y-%m-%d)-$sshcomment-ed25519' && echo '$KeyName' && ssh-keygen -o -a 256 -v -t ed25519 -C '$KeyName' -f '$HOME/.ssh/$KeyName'"
#alias ssh-keygen-ed25519='echo -n "Enter Comment (no_spaces): " && read sshcomment && KeyName="$HOST-$(date +%Y-%m-%d)-$sshcomment-ed25519" && echo "$KeyName" && ssh-keygen -v -o -a 256 -v -t ed25519 -C "$KeyName" -f "$HOME/.ssh/$KeyName"'
#alias ssh-keygen-rsa4096='clear ; echo -n "Enter Comment (no_spaces): " && read sshcomment && clear ; KeyName="$HOST-$(date +%Y-%m-%d)-$sshcomment-rsa4096" && KeyPath="$HOME/.ssh/$KeyName" && echo -e "\nKey:\n\n$KeyPath\n\n" && ssh-keygen -v -t rsa -b 4096 -C "$KeyName" -f "$KeyPath"'
...
alias cat-config-files='fd --color=always --type=file --extension=conf --extension=config --extension=cfg --extension=local --extension=rc --hidden --exclude=skel --no-ignore --exec bat --language=INI --color=always --paging=always --theme=Dracula --italic-text=always --file-name {} --style=full --wrap=never {} | bat --style=grid,header --file-name="CONFIG FILES IN $(pwd)"'
...
alias capslock-escape-keyboard="setxkbmap -layout us -variant ,qwerty -option 'shift:both_capslock_cancel,altwin:menu_win,caps:escape' ; xset r rate 200 30 ; echo -e 'Increased typing speed!\nCapsLock should now be the escape key\t:)'"
# alias swap-capslock-escape-keyboard="setxkbmap -layout us -variant ,qwerty -option 'shift:both_capslock_cancel,altwin:menu_win,caps:escape' ; xset r rate 200 30 ; echo -e 'Increased typing speed!\nCapsLock should now be the escape key\t:)'"
alias swap-capslock-escape-keyboard="setxkbmap -layout us -variant ,qwerty -option 'shift:both_capslock_cancel,altwin:menu_win,caps:escape' ; xset r rate 150 30 ; echo -e 'Increased typing speed!\nCapsLock should now be the escape key\t:)'"
